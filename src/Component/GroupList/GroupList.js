...
import { useDeleteEmailMutation, useGroupsQuery } from '../../features/email/emailApi';
...
const GroupList = () => {
  ...
  const handleEmailByID = async (id) => {
    const res = await axios.get(`http://192.168.50.101:81/email/get_email_by_group/${id}/`);
    setRes(res.data);
      ...
      {groupList && groupList?.map(gl =>
        ...
        <div className="dropdown">
          <div className='drop-head-content-gl'>
            ...
            <label tabIndex={0} className='editBtn-gl' onClick={() => handleEmailByID(gl.id)} ><FaEdit /></label>
